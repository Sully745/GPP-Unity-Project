...
public class S_Lightning : MonoBehaviour
{
    ...
    private void Update()
    {
        if (LightningTest)
        {
            StartCoroutine(_PlayLightning(DurationInSeconds));
            LightningTest = false;
        }
    }

    public void LightningPlay(float duration)
    {
        StartCoroutine(_PlayLightning(duration - 1));
    }

    private IEnumerator _PlayLightning(float duration)
    {
